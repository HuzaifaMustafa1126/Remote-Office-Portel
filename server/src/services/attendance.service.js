...
async function audit(conn, {userId, employeeId, action, entityId, description}) {
  await conn.execute(
...
export async function clockIn(user) {
  return transaction(async conn => {
    ...
    const [[record]] = await conn.execute('SELECT clock_in_at FROM attendance_records WHERE id = ?', [result.insertId]);
    await audit(conn, {userId: user.id, employeeId: user.employee_id, action: 'ATTENDANCE_CLOCK_IN', entityId: result.insertId, description: `${name} clocked in at ${formatAuditTime(record.clock_in_at)}.`});
...
export async function startBreak(user) {
  return transaction(async conn => {
    ...
    if (record.status === 'ON_BREAK') throw new ApiError(409, 'You already have an active break');
    const [[active]] = await conn.execute(`SELECT id FROM attendance_breaks WHERE attendance_id = ? AND status = 'ACTIVE' LIMIT 1 FOR UPDATE`, [record.id]);
...
export async function endBreak(user) {
  return transaction(async conn => {
    ...
    if (record.status === 'CLOCKED_OUT') throw new ApiError(409, 'Your workday has already been completed');
    const [[active]] = await conn.execute(`SELECT id FROM attendance_breaks WHERE attendance_id = ? AND status = 'ACTIVE' ORDER BY break_start_at DESC LIMIT 1 FOR UPDATE`, [record.id]);
...
export async function clockOut(user) {
  return transaction(async conn => {
    ...
    if (record.status === 'CLOCKED_OUT') throw new ApiError(409, 'Your workday has already been completed');
    const [[active]] = await conn.execute(`SELECT id FROM attendance_breaks WHERE attendance_id = ? AND status = 'ACTIVE' LIMIT 1 FOR UPDATE`, [record.id]);
    if (active || record.status === 'ON_BREAK') throw new ApiError(409, 'You must end your break before clocking out');
    await conn.execute(
      `UPDATE attendance_records
...
export async function getToday(user, executor = pool) {
  ...
  const timeline = [{type: 'CLOCK_IN', at: record.clockInAt, label: 'Clocked In'}];
  for (const item of breaks) {
    timeline.push({type: 'BREAK_START', at: item.breakStartAt, label: 'Break Started'});
    if (item.breakEndAt) timeline.push({type: 'BREAK_END', at: item.breakEndAt, label: 'Break Ended'});
  }
...
export async function getHistory(user, filters) {
  ...
  const [rows] = await pool.execute(
    ...
    [employeeId, filters.from || null, filters.from || null, filters.to || null, filters.to || null],
  );
  return rows;
}
...
export async function getAttendance(filters) {
  const [rows] = await pool.execute(
    ...
    [filters.employeeId || null, filters.employeeId || null, filters.department || null, filters.department || null,
      filters.from || null, filters.from || null, filters.to || null, filters.to || null,
...
export async function getDailyReport(date) {
  const [[clock]] = await pool.execute('SELECT COALESCE(?, CURRENT_DATE) AS reportDate', [date || null]);
...
export async function getMonthlyReport(month) {
  const [[clock]] = await pool.execute(`SELECT COALESCE(?, DATE_FORMAT(CURRENT_DATE, '%Y-%m')) AS reportMonth, CURRENT_DATE AS today`, [month || null]);
