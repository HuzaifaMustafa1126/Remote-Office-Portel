...
const select =
  "SELECT ow.id,ow.employee_id employeeId,ow.title,ow.description,ow.status,ow.started_at startedAt,ow.completed_at completedAt,ow.created_at createdAt,ow.updated_at updatedAt,CONCAT(e.first_name,' ',e.last_name) employeeName FROM ongoing_work ow JOIN employees e ON e.id=ow.employee_id";
async function management(user) {
...
export async function mine(user) {
  ...
  const id = employee(user),
    [rows] = await pool.execute(
      `${select} WHERE ow.employee_id=? ORDER BY FIELD(ow.status,'ONGOING','PAUSED','COMPLETED'),ow.updated_at DESC LIMIT 50`,
      [id],
    );
  return rows;
}
export async function team(user) {
  ...
  const [rows] = await pool.execute(
    `${select} WHERE ow.status IN('ONGOING','PAUSED') ORDER BY e.first_name,e.last_name`,
  );
  return rows;
}
export async function create(data, user) {
  if (await management(user))
    throw new ApiError(403, "Only employees can create personal ongoing work");
  const id = employee(user),
    c = await pool.getConnection();
  try {
    await c.beginTransaction();
    await c.execute("SELECT id FROM employees WHERE id=? FOR UPDATE", [id]);
    const [[existing]] = await c.execute(
      "SELECT id FROM ongoing_work WHERE employee_id=? AND status IN('ONGOING','PAUSED') LIMIT 1",
      [id],
    );
    if (existing)
      throw new ApiError(
        409,
        "Complete or delete your current ongoing work before adding another",
      );
    const [r] = await c.execute(
      "INSERT INTO ongoing_work(employee_id,title,description,status,started_at)VALUES(?,?,?,'ONGOING',CURRENT_TIMESTAMP)",
      [id, data.title, data.description || null],
    );
    await c.commit();
    return getOwn(r.insertId, user);
  } catch (e) {
    await c.rollback();
    throw e;
  } finally {
...
async function getOwn(id, user) {
  if (await management(user))
    throw new ApiError(403, "Management cannot modify employee ongoing work");
  const [[row]] = await pool.execute(
    `${select} WHERE ow.id=? AND ow.employee_id=?`,
    [id, employee(user)],
  );
...
export async function update(id, data, user) {
  await getOwn(id, user);
  await pool.execute(
    "UPDATE ongoing_work SET title=?,description=? WHERE id=?",
...
export async function remove(id, user) {
  await getOwn(id, user);
  await pool.execute("DELETE FROM ongoing_work WHERE id=?", [id]);
