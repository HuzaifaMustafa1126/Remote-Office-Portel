...
async function resolveDelivery(userId, type, requested = {}) {
  ...
  return {
    inApp: Boolean(row.globalInApp) && requested.inApp !== false && row.eventInApp !== 0,
    desktop: !dnd && Boolean(row.globalDesktop) && requested.desktop !== false && row.eventDesktop !== 0,
    sound: !dnd && Boolean(row.globalSound) && requested.sound !== false && row.eventSound !== 0,
  };
}
...
export async function getUserNotifications(userId, filters) {
  ...
  if (filters.search) { where.push("(title LIKE ? OR message LIKE ?)"); const q=`%${filters.search}%`; params.push(q,q); }
  if (filters.from) { where.push("created_at>=?"); params.push(`${filters.from} 00:00:00`); }
  if (filters.to) { where.push("created_at<DATE_ADD(?,INTERVAL 1 DAY)"); params.push(`${filters.to} 00:00:00`); }
...
export async function getPreferences(userId) {
  ...
  const [events] = await pool.execute("SELECT event_type eventType,in_app_enabled inAppEnabled,desktop_enabled desktopEnabled,sound_enabled soundEnabled FROM notification_event_preferences WHERE user_id=? ORDER BY event_type",[userId]);
  const normalized=Object.fromEntries(Object.entries(row).map(([key,value])=>[key,key==='volume'?Number(value):Boolean(value)]));
  normalized.eventPreferences=events.map(e=>({...e,inAppEnabled:e.inAppEnabled===null?null:Boolean(e.inAppEnabled),desktopEnabled:e.desktopEnabled===null?null:Boolean(e.desktopEnabled),soundEnabled:e.soundEnabled===null?null:Boolean(e.soundEnabled)}));
  return normalized;
}
...
export async function sendTestNotification(userId) {
  return notifyUser({userId,type:"TEST_NOTIFICATION",title:"Test notification",message:"Your notification system is working correctly.",referenceType:"SYSTEM",eventKey:`TEST_NOTIFICATION:${userId}:${Date.now()}`,priority:"IMPORTANT",delivery:{inApp:true,desktop:true,sound:true}});
}
