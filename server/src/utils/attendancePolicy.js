...
export function classifyArrival(policy, actualMinutes, exempt = false) {
  ...
  if (!exempt && chargeable > 0) {
    if (policy.half_day_rule_enabled && actual > Number(policy.half_day_after_minutes)) status = 'HALF_DAY';
    else if (policy.late_rule_enabled) status = 'LATE';
  }
  return { status, actualMinutes: actual, chargeableMinutes: chargeable,
    countsAsLate: !exempt && Boolean(policy.late_accumulation_enabled) &&
      (status === 'LATE' || (status === 'HALF_DAY' && Boolean(policy.count_half_day_as_late))) };
}
export function convertLateCount(count, policy) {
  ...
  return { remaining: count % threshold, converted: conversions * threshold,
...
export function penaltySalaryDays(type, quantity, policy) {
  ...
  if (type === 'HALF_DAY') return quantity * Number(policy.half_day_salary_deduction_percent) / 100;
...
export function counterPeriod(policy, date, payrollPeriod) {
  if (policy.late_counter_period === 'NEVER') return { start: '1000-01-01', endExclusive: '9999-12-31' };
  ...
  return { start: `${date.slice(0,7)}-01`, endExclusive: new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,1)).toISOString().slice(0,10) };
}
