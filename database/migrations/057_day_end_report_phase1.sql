...
CREATE TABLE IF NOT EXISTS day_end_report_items (
  ...
  UNIQUE KEY uq_day_end_report_task(report_id,task_id),UNIQUE KEY uq_day_end_report_ongoing(report_id,ongoing_work_id),
  INDEX idx_day_end_item_task(task_id),INDEX idx_day_end_item_ongoing(ongoing_work_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
INSERT IGNORE INTO permissions(name,description) VALUES('day_end_report.submit','Submit own Day-End Report');
INSERT IGNORE INTO role_permissions(role_id,permission_id) SELECT r.id,p.id FROM roles r JOIN permissions p ON p.name='day_end_report.submit' WHERE UPPER(r.name)='EMPLOYEE';
INSERT INTO notification_policies(event_type,audience_type,notify_actor,in_app_enabled,desktop_enabled,sound_enabled) VALUES('DAY_END_REPORT_SUBMITTED','CEO_ADMIN',0,1,0,0) ON DUPLICATE KEY UPDATE event_type=VALUES(event_type);
