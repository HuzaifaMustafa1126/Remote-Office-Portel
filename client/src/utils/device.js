...
export function detectDevice({
  ...
  if (
    mobileHint === true ||
    /iPhone|iPod|Android.*Mobile|Windows Phone|Mobi/i.test(userAgent)
  )
    return "mobile";
...
export function getDeviceType() {
  ...
  const ratioSuggestsZoom =
    navigator.maxTouchPoints === 0 &&
    window.outerWidth / Math.max(width, 1) > 1.25;
  ...
  const zoomedDesktop =
    !lastViewport || pixelRatio !== lastViewport.pixelRatio
      ? ratioSuggestsZoom
...
export const canAccessDevice = (user, device) =>
  device !== "mobile" ||
  Boolean(user?.permissions?.includes(MOBILE_PERMISSION));
