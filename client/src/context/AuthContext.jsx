...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    refresh();
    addEventListener("auth:unauthorized", logout);
    return () => removeEventListener("auth:unauthorized", logout);
  }, [refresh, logout]);
  const signIn = async (credentials, remember) => {
    const data = await auth.login(credentials);
    (remember ? localStorage : sessionStorage).setItem("rop_token", data.token);
