...
export default function EmployeeDashboardSidebar({
  ...
  useEffect(() => {
    ...
    getUpcoming()
      .then((days) => {
        if (active) setHolidays(days || []);
      })
      .catch(() => {
        if (active) setHolidaysError("Unable to load upcoming holidays.");
      })
      .finally(() => {
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {showRecent && (
        <section className="rounded-2xl border border-border bg-surface p-5 shadow-sm md:col-span-2">
              ...
              items
                ...
                .map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="grid h-8 w-8 place-items-center rounded-full bg-primary-soft text-primary-text">
                        <Icon size={15} />
                      </span>
