...
export default function EmployeeTeamAvailability({
  data,
  connected,
  activity = [],
}) {
  const people = data?.employees || [],
    counts = data?.counts || {};
  const metrics = [
    ["Online", counts.ONLINE, "bg-success"],
  ...
  return (
    <section className="flex min-h-[360px] min-w-0 flex-col rounded-3xl border border-border bg-surface p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="font-bold">Team Availability</h2>
          <p className="text-xs text-muted-foreground">Live office status</p>
        </div>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-primary-soft px-2.5 py-1 text-[10px] font-bold text-primary-text">
          <i className="h-1.5 w-1.5 rounded-full bg-primary" />
          {connected ? "LIVE" : "CONNECTING"}
        </span>
      </div>
      <div className="mt-4 flex flex-wrap gap-x-3 gap-y-2 rounded-xl bg-surface-secondary/70 p-3">
