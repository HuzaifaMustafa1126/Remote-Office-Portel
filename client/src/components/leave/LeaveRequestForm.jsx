...
import { listDays } from "../../services/companyCalendar.service";
const types = [
  ["CASUAL", "Casual Leave"],
  ["SICK", "Sick Leave"],
...
const count = (a, b, off) => {
  if (!a || !b || b < a) return 0;
  let n = 0,
    d = new Date(`${a}T00:00:00Z`),
    end = new Date(`${b}T00:00:00Z`);
  while (d <= end) {
    const date = d.toISOString().slice(0, 10);
    if (d.getUTCDay() !== 0 && !off.has(date)) n++;
    d.setUTCDate(d.getUTCDate() + 1);
  }
  return n;
};
export default function LeaveRequestForm({ onSubmit, busy }) {
  const [form, setForm] = useState({
    ...
    [off, setOff] = useState(new Set());
  useEffect(() => {
    if (form.startDate && form.endDate)
      listDays({ from: form.startDate, to: form.endDate, status: "ACTIVE" })
        .then((rows) =>
          setOff(
            new Set(
              rows
                .filter((r) => r.dayType !== "WORKING_DAY")
  ...
  const days = useMemo(
    () => count(form.startDate, form.endDate, off),
    [form.startDate, form.endDate, off],
  );
