...
export default function AttendanceStatusCard({
  ...
  const statusCard =
    data?.companyDay && !data.companyDay.isWorkingDay && !record ? (
      <section
        className={`rounded-3xl bg-gradient-to-br from-hero to-hero-end p-8 text-hero-foreground shadow-xl ${showSchedule ? "" : "min-h-[360px]"}`}
      >
      ...
      <section
        className={`overflow-hidden rounded-3xl bg-gradient-to-br from-hero to-hero-end p-6 text-hero-foreground shadow-xl sm:p-8 ${showSchedule ? "" : "min-h-[360px]"}`}
      >
        ...
        <div className="mt-8 grid gap-3 border-t border-sidebar-foreground/10 pt-5 sm:grid-cols-3">
          ...
          {activeBreak && (
            <div>
              ...
              <BreakTimer
                seconds={activeBreak.liveDurationSeconds}
                running
                className="font-mono font-bold text-hero-foreground"
              />
            </div>
          )}
        </div>
