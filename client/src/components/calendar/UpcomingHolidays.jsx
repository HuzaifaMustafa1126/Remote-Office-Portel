      ...
      <div className="mt-3 space-y-2">
        {loading &&
          Array.from({ length: 2 }).map((_, index) => (
            <div
              key={index}
              className="flex animate-pulse items-center gap-3 rounded-xl bg-surface-secondary p-2.5"
            >
        ...
        {!loading && !error && !rows.length && (
          <div className="flex items-center gap-3 py-4">
            <span className="grid h-9 w-9 place-items-center rounded-full bg-surface-secondary text-muted-foreground">
              <CalendarDays size={16} />
            </span>
            <div>
