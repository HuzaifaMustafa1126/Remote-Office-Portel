      ...
      if (child.type === "tr" && inBody) {
        ...
        function cells(items) {
          return Children.map(items, (cell) => {
            ...
            if (cell.type !== "td")
              return cloneElement(cell, {}, cells(cell.props.children));
        ...
        return cloneElement(
          child,
          { role: "row" },
          cells(child.props.children),
        );
      }
      return cloneElement(
        child,
        {},
        decorate(child.props.children, inBody || child.type === "tbody"),
      );
    });
  }
  return (
    <table {...props} role="table" className={`responsive-table ${className}`}>
      {decorate(children)}
    </table>
  );
}
